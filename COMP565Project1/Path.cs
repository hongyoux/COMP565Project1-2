...
namespace AGMGSKv9
{
    ...
    {
        public enum PathType { SINGLE, REVERSE, LOOP, BACKWARDS };
            ...
            : base(theStage)
        {
            ...
            // MODIFICATION : IF aPathType == PathType.BACKWARDS, Add nodes to list of navNodes backwards.
            // Then set aPathType = PathType.LOOP so that it loops backwards instead of forwards.

            // make a simple path, show how to set the type of the NavNode outside of construction.
            node = new List<NavNode>();

            if (aPathType == PathType.BACKWARDS)
            {
                for (int i = pathNode.Length / 2 - 1; i >= 0; i--)
                {
                    x = pathNode[i, 0];
                    z = pathNode[i, 1];
